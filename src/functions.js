const createBoard = (rows, columns) => {
    return Array(rows).fill(0).map((_, rIndex) => {
        return Array(columns).fill(0).map((_, cIndex) => {
            return { row: rIndex, column: cIndex, opened: false, flagged: false, 
                mined: false, exploded: false, nearMines: 0}
        })
    })
}
    ...
    if (!field.opened) {
        ...
        if (field.mined) {
            ...
            getNeighbors(board, row, column).forEach(n => {
                console.log(n)
                return openField(board, n.row, n.column)
            })
        } else {
            const neighbords = getNeighbors(board, row, column)
