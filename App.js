...
export default class App extends Component {
  ...
  onOpenField = (row, column) => {
    ...
    if(lost) {
      showMines(board)
      Alert.alert('Ah, não... Você perdeu!', 'Clique em "Novo Jogo" para iniciar a partida novamente. ')
    }

    if(won){
      Alert.alert('Parabéns! Você ganhou o jogo!', 'Clique em "Novo Jogo" para iniciar a partida novamente. ')
    }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.board}>
          <MineField board={this.state.board} onOpenField={this.onOpenField} onSelectField={this.onSelectField} 
            userLost={this.state.lost} userWon={this.state.won} />
        </View>
      </View>
    );
  }
